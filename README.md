...
|--------------|-------------------------------------|
| Logistic Regression | Best overall performance with 98.25% accuracy and highest AUC (0.9992). Exceptional balance across all metrics with near-perfect precision, recall, and F1 scores. Despite being a simple linear model, it achieves outstanding results on this dataset, indicating strong linear separability between price ranges. The very high MCC (0.9767) demonstrates extremely reliable predictions across all classes. This model is the clear winner for this classification task following Occam’s Razor principle. |
| Decision Tree | Good performance with 86.75% accuracy and strong AUC or class separability (0.9631). The model shows improved generalization with MCC of 0.8235. Lower values (Accuracy, F1, MCC) indicates there could be misclassifications around the threshold. The decent performance across all metrics makes it a solid choice, though it's outperformed by ensemble methods and logistic regression. |
| kNN | Moderate performance with 65% accuracy and the weakest performer. The AUC of 0.8608 suggests reasonable class separablability. The MCC of 0.5334 indicates moderate correlation between predictions and actual values. The distance-based approach struggles with the high-dimensional feature space (20 features), and even with optimized hyperparameters (K=7, distance weighting), it cannot match other algorithms due to the curse of dimensionality. |
| Naive Bayes | Reasonable performance with 81% accuracy and strong AUC or class separability (0.9506). Given the dataset had low correlation between input features satisfying feature indepence, leading to strong and consistent performance across all metrics (precision, recall, F1 all ~0.81) showing balanced predictions across all price range classes. |
| Random Forest (Ensemble) | Random forest, being an ensemble of multiple decission trees showcases higher performance than its base learner with 88.75% accuracy and excellent AUC (0.9817) becausee of its variance reduction. The ensemble approach provides robust generalization with high MCC (0.8500), indicating reliable predictions. By combining multiple decision trees, it reduces overfitting and improves stability compared to a single decision tree. The balanced metrics across precision, recall, and F1 demonstrate consistent performance across all price categories. |
| XGBoost (Ensemble) | Second-best performance with 94% accuracy and very high AUC (0.9927). Excellent MCC (0.9202) demonstrates strong predictive power across all classes. Although the overall relationship between features and the target is approximately linear, gradient boosting improves performance by correcting the errors and tuning the base decission trees sequentially. Shallow trees first capture the dominant linear trends, while subsequent trees correct deviations and boundary cases, resulting in a refined linear approximation with strong regularization to prevent overfitting. |
